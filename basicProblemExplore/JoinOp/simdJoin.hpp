...
struct JoinOp_Unique{
  ...
  static int join(
    ...
    while(pa < lenA || pb < lenB){
      ...
      auto valA = load(predA,va+pa);
      auto valB = load(predB,vb+pb);
      ...
      auto valLeftA = SEPermute<SEPart{OpSrc::A,LRPart::Left}>(matRes,valA,SEPair{da,0u});
      auto valLeftB = SEPermute<SEPart{OpSrc::B,LRPart::Left}>(matRes,valB,SEPair{db,0u});
      store(predLow,vca+pc,valLeftA);
      store(predLow,vcb+pc,valLeftB);
      ...
      if(genC > cpu.v){
        ...
        auto valRightA = SEPermute<SEPart{OpSrc::A,LRPart::Right}>(matRes,valA,SEPair{da,0u});
        auto valRightB = SEPermute<SEPart{OpSrc::B,LRPart::Right}>(matRes,valB,SEPair{db,0u});
        store(predHigh,vca + (pc+cpu.v),valRightA);
        store(predHigh,vcb + (pc+cpu.v),valRightB);
      }
